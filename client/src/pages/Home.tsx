...
const ASSETS = {
  ...
  cover: "/manus-storage/COVER_source_from_Storybook_Page_1-01_203fe040.png",
  spread10: "/manus-storage/Story_Spread_10_b818859b.png",
...
const spreads: Spread[] = [
  ...
  },
  {
    ...
    image: ASSETS.spread10,
    caption: "Reconciled Story Spread 10 — former Spread 11 master",
  ...
  {
    number: 11,
    title: "The New Garden Sign",
    image: ASSETS.spread11,
    caption: "Reconciled Story Spread 11 — former Spread 12 master",
  ...
  },
  {
    ...
    image: ASSETS.spread12,
    caption: "Reconciled Story Spread 12 — former Spread 13 master",
  ...
  {
    number: 13,
    title: "The Secret Leaf Tunnels",
    image: ASSETS.spread13,
    caption: "Reconciled Story Spread 13 — former Spread 14 master",
...
export default function Home() {
  ...
  const isFinalArt = spread.number >= 10;

  const moveSpread = (direction: number) => {
    setFlipDirection(direction > 0 ? "next" : "prev");
    setCurrentNumber((value) => Math.min(13, Math.max(0, value + direction)));
  };
    ...
    <div className="app-shell">
      <aside className="left-rail">
        ...
        <div className="brand-copy" style={{ color: "rgba(248,241,223,.52)", fontSize: 11, lineHeight: 1.5, padding: "0 12px", marginTop: 12 }}>
          Cover + 13 spreads · Ages 6–8<br />Square trim prototype
        </div>
      ...
      <main className="main-area">
        ...
        <div className="main-content">
          ...
          <div className="workspace">
            <section className={`book-stage ${viewMode === "kindle" ? "kindle-view" : ""}`} aria-label="Book spread preview">
              <div className="stage-toolbar">
                <div className="toolbar-group">
                  <button className="text-button" onClick={() => moveSpread(-1)} disabled={currentNumber === 0}><ChevronLeft size={15} /> Previous</button>
                  <button className="text-button" onClick={() => moveSpread(1)} disabled={currentNumber === 13}>Next <ChevronRight size={15} /></button>
                </div>
                <div className="toolbar-group">
                  ...
                  <button className={`text-button ${viewMode === "kindle" ? "primary" : ""}`} onClick={() => setViewMode("kindle")}><Smartphone size={14} /> Kindle frame</button>
                  <button className={`text-button ${viewMode === "desk" ? "primary" : ""}`} onClick={() => setViewMode("desk")}><Monitor size={14} /> Desk view</button>
                </div>
              </div>
              ...
              <div className="thumbnail-row" id="page-map">
                {spreads.map((item) => <button key={item.number} className={`thumb ${currentNumber === item.number ? "active" : ""}`} onClick={() => { setFlipDirection(item.number >= currentNumber ? "next" : "prev"); setCurrentNumber(item.number); }} aria-label={item.number === 0 ? "Open cover" : `Open story spread ${item.number}`}><img src={item.image} alt="" /><div className="thumb-meta"><span>{item.number === 0 ? "CV" : String(item.number).padStart(2, "0")}</span><span style={{ color: item.number === 0 || item.number >= 10 ? "var(--garden)" : "var(--muted-foreground)" }}>{item.number === 0 ? "cover" : item.number >= 10 ? "art" : "ref"}</span></div></button>)}
              </div>
            </section>
